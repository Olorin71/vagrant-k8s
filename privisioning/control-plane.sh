...
if [ ! -d /vagrant/configs ]
then
    mkdir -p /vagrant/configs
fi
# Copy up admin creds to Vagrant share
echo Copying credentials to /vagrant/config/config...
...
# Set up admin creds for user alex
echo Copying credentials to /home/alex...
mkdir -p /home/alex/.kube
cp -i /etc/kubernetes/admin.conf /home/alex/.kube/config
...
#kubectl -n kubernetes-dashboard create token admin-user > /vagrant/configs/dashboard-token
